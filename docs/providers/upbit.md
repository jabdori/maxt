...
| `list_subscriptions(subscription)` | Opens a temporary public connection and preserves the ticket, data types, markets, and optional order-book level returned by `LIST_SUBSCRIPTIONS`. Fixture-verified only | WebSocket request |
