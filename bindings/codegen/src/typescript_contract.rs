use maxt_bindings_common::schema::{ApiType, Identifier, Schema, TaggedUnion, Type};
...
pub(crate) fn render(schema: &Schema) -> String {
    ...
    for provider in schema.providers {
        ...
        members.extend(provider.methods.iter().map(|method| method.name));
        if provider.methods.iter().any(|method| {
            ...
            members.extend(["streamNext", "streamClose"]);
        }
        output.push_str(&format!(
            "  {}: [{}],\n",
            provider.exchange,
