...
모든 내장 어댑터는 인증 정보 없이 공개 시장 데이터를 제공합니다. 공통 작업은
`Client`, 거래소 전용 작업은 `Client::adapter()`를 통해 호출합니다.
...
`Client::new(adapter)` 전에 인증 정보를 추가합니다.
...
`supports(feature) == false`는 인증 정보가 없거나 작업이 구조적으로 미지원이라는
뜻입니다. 호출은 각각 `Error::Auth`, `Error::Unsupported`를 반환합니다.
