...
`Client` wraps an adapter and exposes what the supported exchanges have in
common, so a call means the same thing whichever exchange is underneath.
Anything one exchange offers that the others do not stays on its own adapter.
...
| `limit` on `trades` outside its range | `Error::InvalidRequest`. The ceiling is one call's worth and differs sharply: 1 000 on Binance, 500 on Upbit and Bithumb, 10 on Hyperliquid, whose endpoint takes no count at all. Leave `limit` unset to take what the exchange sends |
...
| `MarketInfo::native_symbol` | what the exchange itself calls the market, for reconciling against its own screen or documentation |
...
**`Unknown` is not untradable.** Upbit and Bithumb designate a market for
investment warning while leaving it tradable, and `MarketStatus` has no value
meaning "trading, but flagged". A second, milder designation does not reach
`MarketStatus` at all.

| Designation | `MarketStatus` | Exchange's own label, verbatim |
| --- | --- | --- |
| warning | `Unknown` | `UpbitAdapter::market_events`, `BithumbAdapter::market_warnings` |
| caution | `Active` | `UpbitAdapter::market_events`, `BithumbAdapter::market_alerts` |

Upbit carries both designations on that one call; Bithumb splits them across
two. [Upbit](providers/upbit.md#warnings-and-cautions) and
[Bithumb](providers/bithumb.md#warnings-and-alerts) define each designation.
...
One contract holds on all four exchanges.
...
| Source of the window's end | Exchanges |
| --- | --- |
...
All four answer the same question, so commit on `closed` alone and do not branch
per exchange.

The Upbit and Hyperliquid candle streams settle a window from the transition:
each subscription holds the window's last frame and emits it with `closed` set
once the exchange opens the next one. Bithumb has no candle stream.

| Case | Behaviour |
| --- | --- |
| the settled emission | repeats the window's `open_time`, so key on `open_time` and overwrite rather than append |
| the frame it carries | Upbit's is from the instant before the next window opens, Hyperliquid's from about two seconds before the window's own close time |
| a subscription you drop | its final window is never settled |
| a window `MarketEvent::Reconnected` interrupts | no settled emission. A reconnect drops what was held rather than settling it across a gap of unknown length |

Exactly one emission per window is why `Overflow::DropNewest` is wrong for
`Feed::Candles`; see [Stream configuration](#stream-configuration).

**An interval names a length, not a grid.** Where a window of that length opens
is the exchange's own decision, and the four disagree at `Hour4`, `Day1`,
`Day3`, `Week1` and `Month1`. `Interval`'s own documentation carries the whole
table.

| Exchange | Grid |
| --- | --- |
| Bithumb | Korean time at every interval, so it matches the others at every interval that divides nine hours and misses at every one that does not, which is `Hour4` and everything daily or longer |
| Hyperliquid | `Day3`, `Week1` and `Month1` measured from the Unix epoch rather than the calendar, so its weeks open on a Thursday and its months are 30-day buckets |

Only `open_time` differs; `closed` answers the same question on all four. Join
two exchanges on `open_time` only where that table says the same of both.
...
`maxt` walks at most a hundred pages. The widest one call can assemble is a
...
too wide and stating the ceiling.
...
To read past the ceiling, set `limit` and step the window yourself.

## Intervals

| Interval set | Guarantee |
| --- | --- |
...
**`Unsupported` on an interval means `maxt` maps no endpoint there**, not that
the exchange never aggregated it. Upbit and Bithumb both serve ten-minute
candles and Upbit serves yearly ones; `Interval` names neither, so `maxt` cannot
ask for them at all. The baseline itself is asserted by `BASELINE_INTERVALS` in
[`tests/unsupported_is_honest.rs`](../tests/unsupported_is_honest.rs).
...
Size an order off `Balance::available`. `locked` is committed to resting orders,
...
| `Size::Base`, `Size::Quote` | names the asset the size is denominated in. A market buy is usually sized in the quote asset and a market sell in the base asset |
...
Both constructors take the size as a `Size`, never a bare number.
...
async fn buy_a_little_bitcoin(client: &Client<UpbitAdapter>) -> maxt::Result<()> {
    ...
    // Priced off the book, never off this page. The deepest bid the exchange
    // returned is below every other bid, so a buy at it cannot cross the ask.
    ...
    if order.status.is_live() {
        // A cancel races the book, so trust the order that comes back.
...
**No limit price written into a document stays safe.**
...
at all. The common API does not carry them. Two adapters expose their exchange's
own answer, both through `Client::adapter`.

| Venue configuration | Source of the rules |
| --- | --- |
...
venue configuration of the five that validates locally. On the other four,
Binance spot included, an order whose price or size is too fine is found out by
the exchange refusing it, which arrives as `Error::Exchange` carrying that
exchange's own code and message. Rounding to fit is the caller's job; nothing in
`maxt` does it, on any of the five.

## Derivatives

Meaningful only on perpetual markets. On a spot-only adapter every one of these
is `Error::Unsupported`. One Hyperliquid adapter carries both kinds, so there
the same calls read as supported and refuse per market: hand one a spot market
and it is `Error::Unsupported` too.

| Call | Result | Credentials |
| --- | --- | --- |
| `positions()`, `positions_on(&market)` | open positions. A row with no size is not one, and `maxt` drops it before answering, whatever the venue publishes | yes |
...
Size a new order off `available_balance`. Each of the three is `Option`, because
not every exchange publishes all three. Read `None` as "the exchange did not
say" and stop: reading it as zero opens nothing, and reading it as unlimited
opens far too much.
...
| `is_flat()` | true when `quantity` is zero. Never true for a position `positions()` handed you, since those are already dropped. It reads a `Position` you built or have been holding since before it closed |
| any unpublished field | `None`, and `None` is not zero |
| `leverage`, `margin_mode` on Binance | always `None`, because the position endpoint `maxt` reads publishes neither. `None` there says nothing about how the position is margined. `maxt` does not subscribe to `ACCOUNT_CONFIG_UPDATE` on the account stream either, so a leverage change does not arrive that way |

Flat rows are dropped on the common API rather than inside each adapter, so the
promise holds for an adapter written against `Adapter` outside this crate too.
Binance is the venue it is needed on: `/fapi/v3/positionRisk` opens a zero-size
row for any symbol that merely carries a resting order, which
[the provider page](providers/binance.md#phantom-positions) measures.
...
async fn size_a_perpetual_order() -> maxt::Result<()> {
    ...
    // `POST /fapi/v1/marginType`. Between them the account sits at the new
    // leverage under the old mode, and Binance routinely refuses a
    // `marginType` change against an open position, which leaves it there.
    // Read the state back before sizing anything off the leverage you asked for.
    client
        .set_margin(
    ...
    for position in client.positions_on(&market).await? {
        // Every one carries size, so there is nothing to skip.
...
at once from anything iterable. Adding the same market or feed twice changes
nothing. Insertion order is kept, and it is the order the exchange is asked in.
...
| `initial_reconnect_delay_ms` | 1 000 | the first backoff, doubling on each consecutive failure; floored at 1 ms |
...
**Nothing resets `max_reconnect_attempts`**: it counts reconnects, not failures.

| Reconnect | Counts against `max_reconnect_attempts` | Resets the backoff |
| --- | --- | --- |
...
| `Feed::Candles` | the bar itself. A window gets [exactly one emission](#candleclosed) with `Candle::closed` set, carrying that window's own final figures. Nothing restates it, so the series is short one bar wherever it is stored |
...
A reconnect failure worth reporting, and the transport error raised when
`max_reconnect_attempts` runs out, both go through the overflow policy, so a
`DropNewest` stream with a full buffer can end having reported nothing at all,
leaving `None` as the only signal that it is over. Size `buffer_size` so that
stays hypothetical.
...
| Feed | Cost of the gap |
| --- | --- |
| `Feed::OrderBook` | nothing. Every book event from every adapter is a self-contained snapshot, several levels deep on each side, never a diff. Overwrite your copy on each event and a reconnect costs one missed message: no sequence number to track and no resynchronization step. How many levels each feed carries is on its provider page |
...
resubscribing. How each exchange authenticates a socket is on
...
buffer, whatever `overflow` is set to.

## Stream termination

Only `None` ends a stream. An `Err` item is a report the stream polls past, so a
...
disconnect, and `maxt`'s own outbound keepalives never push the deadline out.

Each adapter sends its exchange's own keepalive on an interval and raises
`idle_timeout_ms` to a floor its exchange's pace can meet. Ask for longer than a
floor and you get what you asked for; ask for less and you get the floor.

| Exchange | Keepalive interval | Frame sent | Idle floor | Reason for that frame |
| --- | --- | --- | --- | --- |
| Upbit | 15s | text `PING` | 60s | reads every text frame as a command and answers this one, so the keepalive comes back as inbound traffic |
| Bithumb | 15s | text `PING` | 60s | the same |
| Hyperliquid | 15s | `{"method":"ping"}` | 60s | the same |
| Binance | 60s | a protocol ping | 240s | answers an unknown text frame with an error, so the keepalive lives below the API. Its server ping arrives every three minutes and it hangs up only after ten with no pong, so three minutes of silence is a healthy socket there |
...
your program does. Catch `Error::Unsupported` when the answer would end the
program anyway.
...
| Answer | Strength |
| --- | --- |
| `false` | worth trusting. [`tests/unsupported_is_honest.rs`](../tests/unsupported_is_honest.rs) asserts over the whole feature by adapter-configuration cross product that a declined feature refuses as `Unsupported` naming that same feature, never as a transport error, an auth error, or a success |
| `true` | narrower than it looks. The same test can only make the call at one representative argument, so a feature an adapter carries for some arguments and not others still reads `true` |

| Feature reading `true` | Argument it still refuses on |
| --- | --- |
| `Feature::Candles` | an interval outside the exchange's REST set: `Interval::Sec1` on Binance USD-M, for one |
| `Feature::CandleStream` | an interval the exchange serves over REST but does not stream. On Upbit, `Feed::Candles` at `Day1`, `Week1` or `Month1` |
| `Feature::FundingRates`, `Feature::ReduceOnlyOrders` and the rest of the derivatives half on Hyperliquid | a spot market. One Hyperliquid adapter lists both kinds, so the feature is carried by the adapter and not by every market on it |

Branch on the feature to pick an exchange, then handle `Error::Unsupported` at
the call anyway.

## `Error`

| Variant | Meaning | Retry the same request |
| --- | --- | --- |
...
An adapter built without credentials reports `false` from
`client.supports(Feature::Balances)` and fails the call as `Error::Auth`, not
`Error::Unsupported`, before anything reaches the network.
...
The code reaches you intact, and each exchange spells a refused credential
differently:
...
Each provider page lists its own under **Surprises**.
...
| A number `Decimal` cannot hold exactly | `Error::Decode`, never a rounded value. That covers ordinary over-precision and the exponent form exchanges switch into for small values alike: `1e-30` is `Error::Decode`, not a silent zero, and an over-precise `1.234…e5` is `Error::Decode` too |
| Scope of the rule | one shared reader every adapter goes through, so it holds identically on all four |

Convert to floating point at the edge if a calculation genuinely needs it, not
on the way in.
...
| Reason for one unit | exchanges publish seconds, milliseconds, microseconds, and nanoseconds, and one resolution is what lets events from two exchanges be ordered against each other |
...
| Not a `chrono` or `time` type | convert at the edge with `as_nanos` or `into_system_time` |
...
Some payloads carry the read time instead of an exchange clock, and each field
says so in its own documentation. A Binance spot order book and a Hyperliquid
ticker are both like this. Treat such a timestamp as an upper bound on the
data's age; measuring staleness against it will under-report.
...
| **longer than `limit`** | possible on Hyperliquid, where the trim backs up to the start of a run of entries sharing one millisecond rather than stranding the rest of that run past the next cursor. Drop the extras yourself |
...
///
/// Store the string and nothing about its shape: one exchange's cursor is a
...
| Exchange | Published limit | Source |
| --- | --- | --- |
...
| Topic | Detail |
| --- | --- |
| weight against request count | Binance's and Hyperliquid's figures are weight, so what a program can spend depends on which endpoints it calls. Both publish the per-endpoint weight |
| the running total | Binance returns it on every response in an `X-MBX-USED-WEIGHT-(intervalNum)(intervalLetter)` header, `X-MBX-USED-WEIGHT-1M` for the one-minute limiter. `maxt` does not read it, so a program pacing against the budget reads it itself |
...
exchange has beyond the common API. `Client::into_adapter()` unwraps back to the
adapter itself. Both need the concrete adapter type: a `Client<Box<dyn Adapter>>`
gives back `&Box<dyn Adapter>`, which has the common API and nothing more.
