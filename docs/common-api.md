...
| `is_flat()` | the size test, true when `quantity` is zero. Never true for a position `positions()` handed you, since those are already dropped. It reads a `Position` you built or have been holding since before it closed |
...
The flat rows are dropped on the common API rather than inside each adapter, so
the promise holds for an adapter written against `Adapter` outside this crate
too. Binance is the venue it is needed on: `/fapi/v3/positionRisk` opens a
zero-size row for any symbol that merely carries a resting order, which
