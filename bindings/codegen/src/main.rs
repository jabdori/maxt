...
fn main() {
    ...
    outputs.extend([
        ...
        (
            "rust",
            root.join("docs/examples.md"),
            examples::render_catalog(&schema),
        ),
        (
            "rust",
            root.join("examples/README.md"),
            examples::render_directory_index("rust"),
        ),
        (
            "rust",
            root.join("bindings/python/python/maxt/examples/README.md"),
            examples::render_directory_index("python"),
        ),
        (
            "rust",
            root.join("bindings/dart/example/README.md"),
            examples::render_directory_index("dart"),
        ),
        (
            "rust",
            root.join("bindings/typescript/examples/README.md"),
            examples::render_directory_index("typescript"),
        ),
    ]);
...
fn render_markdown(schema: &Schema) -> String {
    examples::validate(schema).expect("every public API must have an example scenario");
    ...
    for operation in schema.adapter_operations {
        let scenario = examples::common(operation.rust_name).expect("validated above");
        output.push_str(&format!(
            "| `{}` | `{}` | [{}](../../../docs/examples.md#{}) |\n",
            operation.rust_name, operation.language_name, scenario.title, scenario.id
        ));
    }
    ...
    for provider in schema.providers {
        for method in provider.methods {
            let scenario =
                examples::provider(provider.exchange, method.rust_name).expect("validated above");
            output.push_str(&format!(
                "| {} | `{}` | `{}` | `{}` | [{}](../../../docs/examples.md#{}) |\n",
                provider.exchange,
                provider.adapter,
...
mod tests {
    ...
    #[cfg(feature = "rust")]
    #[test]
    fn every_public_api_has_a_generated_example_scenario() {
        examples::validate(&binding_schema()).expect("missing example scenario");
    }
