...
`Client`는 어댑터를 감싸 지원 거래소들의 공통 기능을 제공합니다. 밑에 어떤
거래소가 있든 호출의 뜻은 같습니다. 한 거래소에만 있는 기능은 그 거래소의
어댑터에 남습니다.
...
| 거래소가 제공하지 않는 `depth` | `Error::InvalidRequest`. 가까운 깊이로 반올림하지 않습니다. 제공하는 값은 각 거래소 페이지에 있습니다 |
| 범위를 벗어난 `trades`의 `limit` | `Error::InvalidRequest`. 상한은 한 호출이 담는 양이고 편차가 큽니다. Binance 1,000, Upbit과 Bithumb 500, Hyperliquid 10이며 Hyperliquid의 엔드포인트는 개수를 아예 받지 않습니다. 거래소가 주는 만큼 받으려면 `limit`을 비워 두세요 |
| `trades` 정렬 | Binance는 체결 식별자 내림차순이라 여러 체결이 같은 밀리초에 몰려도 정확합니다. Upbit, Bithumb, Hyperliquid는 타임스탬프로 정렬합니다 |
| `MarketInfo::native_symbol` | 거래소 자신이 그 마켓을 부르는 이름. 거래소 화면이나 공식 문서와 대조할 때 씁니다 |
...
| `korean_name`, `english_name` | 거래소가 발행한다면 그 자산의 이름. Binance와 Hyperliquid는 어느 쪽도 발행하지 않아 둘 다 `None`입니다 |

| `MarketStatus` | 뜻 |
| --- | --- |
| `Active` | 상장되어 거래 중 |
| `Paused` | 멈췄지만 상장은 남아 있고 페어는 돌아옵니다 |
| `Delisted` | 사라졌습니다 |
| `Unknown` | 거래소의 답이 나머지 셋 중 어디에도 대응되지 않습니다 |

`MarketStatus`는 `#[non_exhaustive]`이므로 `_` 갈래를 두고 매칭하세요.

**`Unknown`은 거래 불가가 아닙니다.** Upbit과 Bithumb은 거래를 열어 둔 채 유의
종목으로 지정하는데, `MarketStatus`에는 "거래 중이지만 표시가 붙음"에 해당하는
값이 없습니다. 더 약한 지정이 하나 더 있고 이쪽은 `MarketStatus`에 아예 반영되지
않습니다.

| 지정 | `MarketStatus` | 거래소 표기 그대로 읽는 곳 |
| --- | --- | --- |
| 유의 종목 | `Unknown` | `UpbitAdapter::market_events`, `BithumbAdapter::market_warnings` |
| 주의 종목 | `Active` | `UpbitAdapter::market_events`, `BithumbAdapter::market_alerts` |

Upbit은 두 지정을 그 한 호출에 함께 싣고 Bithumb은 둘로 나눠 놓았습니다. 각
지정이 무엇인지는 [Upbit](providers/upbit.ko.md#유의-종목과-주의-종목)과
[Bithumb](providers/bithumb.ko.md#유의-종목과-경보제)에 있습니다.
...
계약은 네 거래소에서 하나이고 거래소가 어떤 순서로 답했든 결과는 오래된
순입니다.
...
네 거래소 모두 캔들의 구간이 끝나면 `true`입니다. 응답의 마지막 캔들만 `false`가
...
| 다음 구간을 여는 프레임. 한 구간의 어떤 프레임도 그 구간의 끝에 닿거나 그보다 뒤로 찍히지 않기 때문입니다 | Upbit과 Hyperliquid 캔들 스트림 |
| 프레임에 실린 거래소 자신의 플래그 | Binance 캔들 스트림 |

넷 다 같은 질문에 답하므로 `closed`만 보고 확정하면 되고 거래소별로 분기할
필요가 없습니다.

Upbit과 Hyperliquid 캔들 스트림은 구간이 바뀌는 시점에 확정합니다. 구독마다 그
구간의 마지막 프레임을 들고 있다가 거래소가 다음 구간을 열면 `closed`를 표시해
내보냅니다. Bithumb에는 캔들 스트림이 없습니다.

| 경우 | 동작 |
| --- | --- |
| 확정 이벤트 | 그 구간의 `open_time`을 다시 싣습니다. `open_time`을 키로 삼아 덧붙이지 말고 덮어쓰세요 |
| 확정 이벤트가 싣는 프레임 | Upbit은 다음 구간이 열리기 직전 것, Hyperliquid는 그 구간의 마감 시각보다 약 2초 앞선 것입니다 |
| 구독을 끊었을 때 | 마지막 구간은 확정되지 않습니다 |
| `MarketEvent::Reconnected`가 끊고 지나간 구간 | 확정 이벤트가 없습니다. 재연결은 들고 있던 것을 길이를 알 수 없는 공백 너머로 확정하지 않고 버립니다 |

구간마다 확정 이벤트가 정확히 하나뿐이라서 `Feed::Candles`에는
`Overflow::DropNewest`가 맞지 않습니다. [스트림 설정](#스트림-설정)을 보세요.

**간격은 길이의 이름이지 격자의 이름이 아닙니다.** 그 길이의 구간이 어디서
열리는지는 거래소가 정합니다. `Hour4`, `Day1`, `Day3`, `Week1`, `Month1`에서 네
곳의 답이 갈립니다. 전체 표는 `Interval` 자신의 문서에 있습니다.

| 거래소 | 격자 |
| --- | --- |
| Bithumb | 모든 간격을 한국 시간으로 자릅니다. 9시간을 나누어떨어지게 하는 간격에서는 나머지 셋과 일치하고 그렇지 않은 간격에서는 어긋납니다. 어긋나는 간격은 `Hour4`와 하루 이상의 모든 간격입니다 |
| Hyperliquid | `Day3`, `Week1`, `Month1`을 달력이 아니라 Unix 에포크에서부터 잽니다. 그래서 주는 목요일에 열리고 월은 30일 구간입니다 |

`open_time`만 다르고 `closed`는 네 곳 모두 같은 질문에 답합니다. 두 거래소를
`open_time`으로 조인하려면 그 표가 양쪽에 같은 말을 해야 안전합니다.
...
이 거래소들은 하나같이 한 응답씩 뒤로 거슬러 페이지를 넘깁니다. 그래서 넓은
구간은 페이지마다 순차 왕복 한 번을 쓰고 이를 빠르게 만들 방법이 없습니다.
`maxt`는 최대 100페이지를 걸어가므로 한 호출로 모을 수 있는 최대치는 거래소의
응답당 상한에 100을 곱한 값입니다. 한 번에 200개인 Upbit이라면 캔들 2만
개쯤입니다. 그보다 더 요구하면 **첫 호출 전에** `Error::InvalidRequest`가 되고
구간을 넓힌 필드의 이름과 한계치를 함께 알려 줍니다.
...
한계치보다 넓게 읽으려면 `limit`을 정해 두고 구간을 직접 옮기세요.

## 간격

| 간격 집합 | 보장 |
| --- | --- |
| 기준선 | `client.supports(Feature::Candles) == true`는 모든 거래소에서 열 개를 보장합니다. `Min1`, `Min3`, `Min5`, `Min15`, `Min30`, `Hour1`, `Hour4`, `Day1`, `Week1`, `Month1`입니다. 이 열 개를 기준으로 쓰면 같은 코드가 네 거래소에서 모두 캔들을 읽습니다. **네 곳에서 같은 격자를 읽는다는 뜻은 아닙니다.** `Hour4`, `Day1`, `Week1`, `Month1`에서는 여는 시각이 거래소마다 다릅니다. 어디가 다른지는 `Interval`이 문서로 남깁니다 |
...
**간격에 붙은 `Unsupported`는 `maxt`가 그곳에 엔드포인트를 대응시켜 두지
않았다는 뜻**이지 거래소가 그 간격을 집계한 적이 없다는 뜻이 아닙니다. Upbit과
Bithumb은 둘 다 10분봉을 제공하고 Upbit은 연봉까지 제공하지만 `Interval`이 어느
쪽도 이름 붙이지 않아 `maxt`로는 요청할 방법이 아예 없습니다. 기준선 자체는
[`tests/unsupported_is_honest.rs`](../tests/unsupported_is_honest.rs)의
`BASELINE_INTERVALS`가 검증합니다.
...
주문 크기는 `Balance::available`을 기준으로 잡으세요. `locked`는 걸려 있는
주문에 이미 묶인 몫이고 그것까지 쓰려 들면 거래소가 거절합니다.
...
| `Size::Base`, `Size::Quote` | 크기가 어느 자산 기준인지 밝힙니다. 시장가 매수는 보통 quote 자산 기준, 시장가 매도는 기준 자산 기준입니다 |
...
두 생성자 모두 크기를 `Size`로 받습니다. 맨 숫자는 받지 않습니다.
...
async fn buy_a_little_bitcoin(client: &Client<UpbitAdapter>) -> maxt::Result<()> {
    ...
    // 가격은 이 문서가 아니라 호가창에서 가져옵니다. 거래소가 돌려준 가장 깊은
    // 매수 호가는 다른 어떤 매수 호가보다 낮으므로 그 자리의 매수는 매도 호가를
    // 건드리지 못합니다.
    ...
    if order.status.is_live() {
        // 취소는 호가창과 경합하므로 넣은 주문이 아니라 돌아온 주문을 믿으세요.
...
**문서에 적어 둔 지정가는 어느 것도 계속 안전하지 않습니다.**

| 요건 | 방법 |
| --- | --- |
| 가격 정하기 | 조금 전에 읽은 호가창을 기준으로, 매수는 최우선 매수 호가 이하로, 매도는 최우선 매도 호가 이상으로 잡으세요 |
| 보호하기 | `TimeInForce::PostOnly`를 쓰세요. 그 사이 호가창이 움직였다면 거래소가 체결하지 않고 거절합니다 |
| 하지 말 것 | 이 문서를 포함해 어떤 문서에서도 가격을 베껴 오지 마세요. 여기 있는 숫자는 실행할 때 거래소에서 읽어 온 값이거나 수량입니다. 가격인 적은 없습니다 |

## 주문 정밀도와 최소 주문 크기

호가 단위, 수량 단위, 최소 주문 금액이 주문의 접수 여부를 결정합니다. 공통 API는
이 셋을 싣지 않습니다. 어댑터 둘이 자기 거래소의 답을 그대로 내놓고 둘 다
`Client::adapter`로 닿습니다.
...
**규칙을 노출하는 것과 규칙에 맞춰 검사하는 것은 다릅니다.** 다섯 거래소 구성 중
Hyperliquid만 로컬에서 검사합니다. Binance 현물을 포함한 나머지 넷에서는
거래소가 주문을 거절하고 나서야 가격이나 크기가 너무 잘게 쪼개졌다는 사실을 알게
됩니다. 그 거절은 거래소 자신의 코드와 메시지를 담은 `Error::Exchange`로
도착합니다. 규격에 맞춰 반올림하는 일은 호출자가 합니다. `maxt`는 다섯 구성
어디에서도 대신 해 주지 않습니다.

## 파생상품

무기한 선물 마켓에서만 의미가 있습니다. 현물 전용 어댑터에서는 아래 모두가
`Error::Unsupported`입니다. Hyperliquid 어댑터 하나는 두 종류를 함께 다루므로
그곳에서는 같은 호출들이 지원되는 것으로 읽히고 마켓별로 거절합니다. 현물 마켓을
건네면 그때도 `Error::Unsupported`입니다.

| 호출 | 결과 | 인증 정보 |
| --- | --- | --- |
| `positions()`, `positions_on(&market)` | 보유 포지션. 크기가 0인 행은 포지션이 아닙니다. 거래소가 무엇을 게시하든 `maxt`가 답하기 전에 버립니다 | 필요 |
...
새 주문의 크기는 `available_balance`를 기준으로 잡으세요. 셋 다 `Option`입니다.
모든 거래소가 셋을 다 발행하지는 않기 때문입니다. `None`은 "거래소가 말하지
않았다"로 읽고 거기서 멈추세요. 0으로 읽으면 아무것도 열지 못하고 무제한으로
읽으면 지나치게 많이 엽니다.
...
| `is_flat()` | `quantity`가 0일 때 참입니다. `positions()`가 건넨 포지션에서는 절대 참이 아닙니다. 그런 행은 이미 버려졌기 때문입니다. 직접 만든 `Position`이나 닫히기 전부터 들고 있던 `Position`을 읽는 용도입니다 |
| 발행되지 않는 모든 필드 | `None`이고 `None`은 0이 아닙니다 |
| Binance의 `leverage`, `margin_mode` | 언제나 `None`입니다. `maxt`가 읽는 포지션 엔드포인트가 두 필드를 발행하지 않기 때문입니다. 그곳의 `None`은 포지션에 마진이 어떻게 걸렸는지를 두고 아무 말도 하지 않습니다. 계정 스트림의 `ACCOUNT_CONFIG_UPDATE`도 구독하지 않으므로 레버리지 변경은 그쪽으로도 오지 않습니다 |

크기가 0인 행은 어댑터마다가 아니라 공통 API에서 버립니다. 그래서 이 크레이트
바깥에서 `Adapter`를 구현한 어댑터에도 이 약속이 그대로 적용됩니다. 이 약속은
Binance 때문에 필요합니다. `/fapi/v3/positionRisk`는 미체결 주문만 얹힌 심볼에도
크기가 0인 행을 여는데, 이는 [거래소
문서](providers/binance.ko.md#유령-포지션)가 측정한 내용입니다.
...
async fn size_a_perpetual_order() -> maxt::Result<()> {
    ...
    // 요청은 하나지만 Binance 호출은 둘입니다. `POST /fapi/v1/leverage`가 먼저
    // 나가고 `POST /fapi/v1/marginType`이 그다음입니다. 그 사이에 계좌는 새
    // 레버리지와 예전 모드에 놓이는데, 포지션이 열린 상태의 `marginType` 변경을
    // Binance가 흔히 거절하므로 그대로 남을 수 있습니다. 요청한 레버리지를 믿고
    // 크기를 잡기 전에 상태를 다시 읽으세요.
    client
        .set_margin(
...
`.feed(..)`을 붙입니다. 여러 마켓을 한꺼번에 넣을 때는 순회 가능한 값을 받는
`.markets_iter(..)`을 씁니다. 같은 마켓이나 피드를 두 번 넣어도 달라지지
않습니다. 넣은 순서는 그대로 남고 거래소에 묻는 순서가 바로 그 순서입니다.
...
평범한 구조체이므로 달라질 필드만 지정하고 나머지는
`..StreamConfig::default()`로 물려받으세요.
...
| `initial_reconnect_delay_ms` | 1 000 | 첫 백오프. 연속 실패 한 번마다 두 배가 되며 1 ms를 하한으로 둡니다 |
| `max_reconnect_delay_ms` | 30 000 | 두 배로 늘어나다 멈추는 상한. 1 ms를 하한으로 둡니다 |
| `idle_timeout_ms` | 30 000 | 어댑터가 올릴 수 있는 하한, [하트비트](#하트비트) 참고 |

**`max_reconnect_attempts`를 되돌리는 것은 없습니다.** 실패가 아니라 재연결을
셉니다.

| 재연결 | `max_reconnect_attempts` 차감 | 백오프 초기화 |
| --- | --- | --- |
| 아예 열리지 않았다 | 차감 | 안 함 |
| 소켓은 열렸지만 거래소가 아무것도 보내지 않았다 | 차감 | 안 함 |
| 열린 소켓으로 프레임이 하나라도 왔다 | 차감 | 함 |
...
| 거래소가 모든 연결에 거절로 답하는데 기본값 `None`입니다 | 거절도 프레임이라 백오프를 되돌립니다. 프로세스가 사는 내내 `initial_reconnect_delay_ms` 간격으로 계속 재연결합니다. 그 응답은 소비자가 직접 읽어야 하고 한도를 묶는 쪽은 `max_reconnect_attempts`입니다 |
| 서버가 연결은 받아 주고 아무 말도 하지 않습니다 | 상한까지 백오프합니다. 재연결 세 번이 연달아 살아 있는 연결을 남기지 못하면 전송 에러로 보고하고, 닿지 않는 서버와 똑같이 묶입니다 |

**어떤 지연 필드도 무엇을 세는지는 정하지 않습니다.** `max_reconnect_delay_ms`를
올리면 재시도가 더 느긋해질 뿐입니다.
...
| 가장 오래된 것을 버리는 정책은 없음 | 보내는 쪽에서는 가득 찬 큐의 앞쪽을 축출하지 못합니다. 최신 우선이 필요하면 스트림을 촘촘한 루프로 비우면서 마지막에 본 이벤트만 남기세요 | 없습니다. 그런 정책 자체가 없습니다 |

| 피드 | `DropNewest`의 대가 |
| --- | --- |
| `Feed::Ticker`, `Feed::OrderBook` | 낡은 값뿐입니다. 이벤트 하나하나가 완결된 현재 값이고 다음 이벤트가 그것을 다시 말합니다 |
| `Feed::Trades` | 조용히 모자란 합계입니다. 체결 하나하나가 뒤에 반복되지 않는 별개의 사실입니다 |
| `Feed::Candles` | 봉 자체입니다. 구간 하나에는 `Candle::closed`가 표시된 [확정 이벤트가 하나뿐](#candleclosed)이고 그 구간의 최종 수치를 싣습니다. 다시 말해 주는 이벤트가 없으므로 저장한 시계열은 봉 하나가 빈 채로 남습니다 |

**버려진 이벤트는 흔적을 남기지 않고 에러도 같은 방식으로 버려집니다.**
`DropNewest`는 계수기도, 스트림에 실리는 이벤트도, 로그 한 줄도 없이 조용히
버립니다. 보고할 만한 재연결 실패도, `max_reconnect_attempts`가 소진될 때 나는
전송 에러도 오버플로 정책을 거칩니다. 그래서 버퍼가 가득 찬 `DropNewest`
스트림은 아무것도 보고하지 않은 채 끝나고 `None`만이 끝났다는 유일한 신호로
남기도 합니다. `buffer_size`는 그 일이 가정으로만 남을 만큼 잡으세요.

**예외는 재연결 소식입니다.** `MarketEvent::Reconnected`와
`AccountEvent::Reconnected`는 버퍼가 가득하면 버려지지 않고 붙들려 있다가 자리가
나는 첫 이벤트보다 앞서 전달됩니다. 기다리지는 않습니다. 자리가 날 때까지 연결은
계속 읽고 데이터는 계속 버립니다.
...
| 피드 | 빈 구간의 대가 |
| --- | --- |
| `Feed::OrderBook` | 없습니다. 모든 어댑터의 모든 호가창 이벤트는 차분이 아니라 양쪽으로 여러 단계를 담은 완결된 스냅숏입니다. 이벤트마다 사본을 덮어쓰면 재연결의 비용은 놓친 메시지 하나입니다. 따라갈 순번도 재동기화 단계도 없습니다. 피드가 몇 단계를 싣는지는 각 거래소 페이지에 있습니다 |
| `Feed::Trades` | 진짜 구멍이 생깁니다. 체결이 곧 그 순서이고 끊긴 사이에 발행된 것은 다시 오지 않습니다. `trades`로 REST에서 메우세요. 거래소가 채워 주는 곳에서는 겹치는 구간을 `Trade::id`로 중복 제거하면 됩니다. Binance에서는 정확히 맞습니다. 두 거래 시장 모두 `@trade`와 `/trades`가 같은 체결을 같은 식별자로 부릅니다. Hyperliquid는 최근 10건만 주므로 그보다 넓은 공백은 그곳에서 구멍으로 남습니다 |

구독 복구에는 거래소가 요구하는 경우의 재인증도 들어갑니다. 그래서 비공개
스트림은 호출자가 다시 구독하지 않아도 재연결을 넘깁니다. 각 거래소가 소켓을
어떻게 인증하는지는 [Upbit](providers/upbit.ko.md#인증-정보)과
...
`None`만이 스트림을 끝냅니다. `Err` 항목은 스트림이 지나쳐 가며 보고하는
내용입니다. 첫 `Err`에서 루프를 빠져나가면 곧 회복될 스트림을 버리는 셈입니다.
`None`은 `StreamConfig::max_reconnect_attempts`가 소진되었을 때, 또는 스트림이
드롭된 뒤에 옵니다. `MarketStream`과 `AccountStream` 모두 이렇게 동작합니다.
...
그 프레임이 소비자에게 닿으면 한 번 더 걸립니다. 그래서 소비자가
`Overflow::Backpressure`를 붙들 만큼 느려도 연결이 끊기지 않고 `maxt`가 스스로
내보내는 keepalive도 기한을 밀어내지 못합니다.

각 어댑터는 자기 거래소의 keepalive를 일정 간격으로 보내고 자기 거래소의 속도가
맞출 수 있는 하한까지 `idle_timeout_ms`를 올립니다. 하한보다 길게 요청하면
요청한 값을 받고 짧게 요청하면 하한을 받습니다.

| 거래소 | keepalive 간격 | 보내는 프레임 | 유휴 하한 | 그 프레임을 쓰는 이유 |
| --- | --- | --- | --- | --- |
| Upbit | 15초 | 텍스트 `PING` | 60초 | 모든 텍스트 프레임을 명령으로 읽고 이것을 처리하므로 keepalive가 수신 트래픽으로 돌아옵니다 |
| Bithumb | 15초 | 텍스트 `PING` | 60초 | 같습니다 |
| Hyperliquid | 15초 | `{"method":"ping"}` | 60초 | 같습니다 |
| Binance | 60초 | 프로토콜 ping | 240초 | 알 수 없는 텍스트 프레임에 에러로 답하므로 keepalive가 API 아래에 있습니다. 서버 ping을 3분마다 보내고 pong이 10분 동안 오지 않을 때에만 끊으므로 그곳에서 3분의 침묵은 건강한 소켓입니다 |

## `Feature`와 `Client::supports`

`client.supports(feature)`는 인증 정보를 넣었는지까지 포함해 지금 구성된
어댑터를 기준으로 요청 없이 로컬에서 답합니다. 그 답에 따라 프로그램이 다르게
움직여야 할 때 물어보세요. 어차피 프로그램이 끝날 상황이라면
`Error::Unsupported`를 잡으세요.
...
**`supports`는 기능에 답하지 여러분이 건넨 인자에는 답하지 않습니다.**

| 답 | 신뢰 범위 |
| --- | --- |
| `false` | 믿을 만합니다. [`tests/unsupported_is_honest.rs`](../tests/unsupported_is_honest.rs)는 기능과 어댑터 구성의 곱집합 전체를 돌면서 거절한 기능이 전송 에러도 인증 에러도 성공도 아닌 바로 그 기능을 지목하는 `Unsupported`로 거절하는지 단언합니다 |
| `true` | 보이는 것보다 좁습니다. 같은 테스트가 호출은 대표 인자 하나로만 해 볼 수 있습니다. 그래서 어떤 인자에는 있고 어떤 인자에는 없는 기능도 여전히 `true`로 읽힙니다 |

| `true`로 읽히는 기능 | 여전히 거절하는 인자 |
| --- | --- |
| `Feature::Candles` | 거래소의 REST 간격 집합 밖의 간격. 예를 들어 Binance USD-M의 `Interval::Sec1` |
| `Feature::CandleStream` | 거래소가 REST로는 제공하지만 스트림으로는 보내지 않는 간격. Upbit의 `Day1`, `Week1`, `Month1` `Feed::Candles`가 그렇습니다 |
| Hyperliquid의 `Feature::FundingRates`, `Feature::ReduceOnlyOrders`를 비롯한 파생상품 쪽 전부 | 현물 마켓. Hyperliquid 어댑터 하나가 두 종류를 함께 상장하므로 기능은 어댑터가 싣지 그 위의 모든 마켓이 싣지 않습니다 |

기능으로는 거래소를 고르되, 호출 지점에서는 그래도 `Error::Unsupported`를
처리하세요.
...
인증 정보 없이 만든 어댑터는 `client.supports(Feature::Balances)`에 `false`를
답하고 호출 자체도 네트워크에 닿기 전에 `Error::Unsupported`가 아니라
`Error::Auth`로 실패합니다.
...
코드는 그대로 전달되고 거부된 인증 정보를 표기하는 방식은 거래소마다 다릅니다.
...
각 거래소 문서의 **뜻밖의 것들**에 그 거래소의 목록이 있습니다.

### `is_retryable()`과 수신 창을 벗어난 시각

`is_retryable()`은 *똑같은* 요청을 다시 보내면 성공할 수 있느냐를 확인합니다.
서명된 요청은 서명할 때의 시각을 그대로 지니므로 거래소가 수신 창을 벗어났다고
거부한 시각은 시계 오차가 일시적인 조건인데도 여기서 `false`입니다.

| 원인 | 해결 |
| --- | --- |
| 이 기계의 시계가 틀림 | 시계를 맞추세요. 맞출 때까지 다시 만든 요청도 매번 실패합니다. 재시도 루프는 그 사실을 배우는 데 예산을 다 씁니다 |
...
| 항목 | 뜻 |
| --- | --- |
| 모든 가격, 수량, 금액 | `rust_decimal::Decimal`입니다. 호출자가 실수로 다른 버전을 쓰는 일이 없도록 `maxt::Decimal`로 다시 내보냅니다 |
| `Decimal`이 정확히 담지 못하는 숫자 | 반올림된 값이 아니라 `Error::Decode`입니다. 자릿수가 넘치는 평범한 경우와 거래소가 작은 값에 쓰는 지수 표기가 똑같이 여기에 들어갑니다. `1e-30`은 조용한 0이 아니라 `Error::Decode`이고 자릿수가 넘치는 `1.234…e5`도 `Error::Decode`입니다 |
| 규칙의 적용 범위 | 모든 어댑터가 거쳐 가는 하나의 공용 리더이므로 네 거래소에서 똑같이 성립합니다 |

부동소수점이 정말로 필요한 계산이라면 들어오는 길목이 아니라 경계에서
변환하세요.
...
| `chrono`나 `time`의 타입이 아님 | `as_nanos`나 `into_system_time`으로 경계에서 변환하세요 |
| 값을 지어내야만 채울 수 있는 필드 | 대신 `None`. Binance와 Hyperliquid에서 `Ticker::last_trade_time`이 `None`인 것은 둘 다 마지막 가격이 언제 체결됐는지 말하지 않기 때문입니다 |

어떤 응답은 거래소 시계 대신 읽은 시각을 담고 각 필드의 문서에 그렇게 적혀
있습니다. Binance 현물 호가창과 Hyperliquid 티커가 그렇습니다. 그런 타임스탬프는
데이터 나이의 상한으로 다루세요. 이 값으로 신선도를 재면 실제보다 짧게 나옵니다.
...
앞의 둘을 먼저 쓰세요. 종류가 설정이나 명령줄에서 온다면 타입 있는 어댑터
메서드를 잃어 가면서까지 `Client<Box<dyn Adapter>>`를 쓸 만합니다.
...
같은 페어의 현물과 무기한 선물은 서로 다른 마켓입니다. 플래그 하나가 다른 한
마켓이 아닙니다. 가격이 다르고 호가창이 다르며 Binance에서는 호스트도 잔고도
다릅니다.
...
| **`limit`보다 길 때** | Hyperliquid에서 가능합니다. 같은 밀리초를 공유하는 항목 묶음 안에서 자르면 그 나머지가 다음 커서 너머로 밀려납니다. 그래서 묶음이 시작하는 자리까지 물러나 자릅니다. 남는 항목은 직접 버리세요 |

버퍼 크기는 페이지가 돌려준 개수에 맞춰 잡으세요. `limit`이 기준이 아닙니다.
...
///
/// 문자열만 저장하고 그 생김새는 저장하지 마세요. 어느 거래소의 커서는
/// 타임스탬프이고 어느 거래소의 커서는 주문 식별자이며 둘 다 약속은 아닙니다.
...
백오프도 없습니다. 호출하면 그대로 나가고 속도 조절은 여러분이 합니다.
...
| 항목 | 내용 |
| --- | --- |
| 가중치와 요청 수 | Binance와 Hyperliquid의 값은 가중치이므로 프로그램이 실제로 쓸 수 있는 양은 어떤 엔드포인트를 부르는지에 달려 있습니다. 두 거래소 모두 엔드포인트마다 가중치를 공개합니다 |
| 누적값 | Binance는 응답마다 `X-MBX-USED-WEIGHT-(intervalNum)(intervalLetter)` 헤더로 돌려주고 1분 제한기의 헤더는 `X-MBX-USED-WEIGHT-1M`입니다. `maxt`는 이 값을 읽지 않으므로 예산에 맞춰 조절하려는 프로그램은 직접 읽습니다 |
...
| 묶어 부를 수 있는 곳 | Upbit에 티커 서른 개를 한 요청으로 물으면 초당 열 번 중 서른 번이 아니라 한 번을 씁니다. [`Client::adapter`](#clientadapter)로 하는 일입니다. 그 마켓 목록의 길이에 `maxt`도 상한을 두지 않고 Upbit도 상한을 공시하지 않습니다. URL이 버거울 만큼 긴 목록은 Upbit이나 그 앞단이 거절하고 `Error::Exchange`로 도착합니다. 쓸 수 있는 상한은 직접 찾으세요. [Upbit](providers/upbit.ko.md#upbit-전용-호출)을 보세요 |

## `Client::adapter`

`Client::adapter()`는 어댑터를 그대로 돌려주고 그 거래소가 공통 API 밖에 둔 타입
있는 메서드들을 함께 넘겨줍니다. `Client::into_adapter()`는 어댑터 자체로 되돌려
줍니다. 둘 다 구체 어댑터 타입이 있어야 합니다. `Client<Box<dyn Adapter>>`는
`&Box<dyn Adapter>`를 돌려주고 거기에는 공통 API밖에 없습니다.
